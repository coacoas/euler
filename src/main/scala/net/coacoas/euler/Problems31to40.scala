...
object P37 extends Problem {
  lazy val primeSet = Streams.primes.takeWhile(_< 10000000).toSet
  def isPrime(n: Long) = primeSet.contains(n)

  def truncateLeft(n: Long) = n.toString.toList.tail.mkString.toLong
  def truncateRight(n: Long): Long = n / 10

  def isTruncatable(n: Long, f: Long => Long) = {
    @tailrec
    def iter(remaining: Long): Boolean = 
      if (remaining < 10) isPrime(remaining)
      else isPrime(remaining) && iter(f(remaining))

    iter(n)
  }

  override def run = 
    Streams.primes.dropWhile(_ < 10).
      filter(isTruncatable(_, truncateLeft)).
      filter(isTruncatable(_, truncateRight)).
      take(11).sum
}